...
MedicalRecord.uploadRecordFile = (medicalRecord, result) =>{
    sql.query("Update MedicalRecord set fileStore = ? where id = ?", [medicalRecord.fileStore, medicalRecord.id], (err, res) =>{
        if(err){
            result(err, null);
            return;
        }
...
MedicalRecord.uploadRecordPredistion = (medicalRecord, result) =>{
    sql.query("Update MedicalRecord set precription ? where id = ?", [medicalRecord.precription, medicalRecord.id], (err, res) =>{
        if(err){
            result(err, null);
            return;
        }
