...
exports.create = (req, res) => {
  ...
  const bmi = new BMI({
    ...
    bmi: req.body.weigh / (0.01 * req.body.tall * 0.01 * req.body.tall),
  });
...
exports.findAll = (req, res) => {
  BMI.getAll((err, data) => {
    if (err)
      res.status(500).json({
        message: err.message || "Some error occurred while retrieving BMI.",
        bmi: null,
        count: 0,
      });
    else
  ...
  BMI.findByPatientId(req.params.patientId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).json({
          message:
            "Error retrieving BMI with patient id: " + req.params.patientId,
          bmi: null,
          count: 0,
        });
      }
...
exports.deleteAll = (req, res) => {
  BMI.removeAll((err, data) => {
    if (err) {
      res.status(500).json({
        message: err.message || "Some error occurred while removing all BMI.",
        bmi: null,
        count: 0,
      });
